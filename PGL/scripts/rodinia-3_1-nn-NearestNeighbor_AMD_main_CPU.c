...
int i;
const int LIST_SIZE = 513168;
float2* a = (float2*) malloc(sizeof(float2) * LIST_SIZE);
for (int aaa = 0; aaa < LIST_SIZE; aaa++) {
	a[aaa] = (float2) ( 5.0, 5.0);
}
...
fp = fopen("rodinia-3.1-nn-NearestNeighbor.cl", "r");
if (!fp) {
...
ret = clEnqueueWriteBuffer(command_queue, a_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float2), a, 0, NULL, NULL);
...
ret = clSetKernelArg(kernel, 3, sizeof(cl_float), &d);
