...
float4* d = (float4*) malloc(sizeof(float4) * LIST_SIZE);
for (int ddd = 0; ddd < LIST_SIZE; ddd++) {
	d[ddd] = (float4) ( 5.0, 5.0, 5.0, 5.0);
}
...
float4* f = (float4*) malloc(sizeof(float4) * LIST_SIZE);
for (int fff = 0; fff < LIST_SIZE; fff++) {
	f[fff] = (float4) ( 5.0, 5.0, 5.0, 5.0);
}
...
fp = fopen("rodinia-3.1-lavaMD-kernel_gpu_opencl.cl", "r");
if (!fp) {
...
ret = clEnqueueWriteBuffer(command_queue, d_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float4), d, 0, NULL, NULL);
...
ret = clEnqueueWriteBuffer(command_queue, f_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float4), f, 0, NULL, NULL);
