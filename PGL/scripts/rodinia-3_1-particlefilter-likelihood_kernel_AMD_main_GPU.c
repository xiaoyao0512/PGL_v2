#include<stdio.h>
#include<stdlib.h>
#include<sys/time.h>
#ifdef __APPLE__
#include <OpenCL/opencl.h>
#else
#include <CL/cl.h>
#endif

#define MAX_SOURCE_SIZE (0x100000)

int main(void) {

int i;
const int LIST_SIZE = 126564393;
float* a = (float*) malloc(sizeof(float) * LIST_SIZE);
for (int aaa = 0; aaa < LIST_SIZE; aaa++) {
	a[aaa] = 5.0;
}

float* b = (float*) malloc(sizeof(float) * LIST_SIZE);
for (int bbb = 0; bbb < LIST_SIZE; bbb++) {
	b[bbb] = 5.0;
}

float* c = (float*) malloc(sizeof(float) * LIST_SIZE);
for (int ccc = 0; ccc < LIST_SIZE; ccc++) {
	c[ccc] = 5.0;
}

float* d = (float*) malloc(sizeof(float) * LIST_SIZE);
for (int ddd = 0; ddd < LIST_SIZE; ddd++) {
	d[ddd] = 5.0;
}

float* e = (float*) malloc(sizeof(float) * LIST_SIZE);
for (int eee = 0; eee < LIST_SIZE; eee++) {
	e[eee] = 5.0;
}

int* f = (int*) malloc(sizeof(int) * LIST_SIZE);
for (int fff = 0; fff < LIST_SIZE; fff++) {
	f[fff] = 5;
}

int* g = (int*) malloc(sizeof(int) * LIST_SIZE);
for (int ggg = 0; ggg < LIST_SIZE; ggg++) {
	g[ggg] = 5;
}

float* h = (float*) malloc(sizeof(float) * LIST_SIZE);
for (int hhh = 0; hhh < LIST_SIZE; hhh++) {
	h[hhh] = 5.0;
}

int* i = (int*) malloc(sizeof(int) * LIST_SIZE);
for (int iii = 0; iii < LIST_SIZE; iii++) {
	i[iii] = 5;
}

float* j = (float*) malloc(sizeof(float) * LIST_SIZE);
for (int jjj = 0; jjj < LIST_SIZE; jjj++) {
	j[jjj] = 5.0;
}

float* k = (float*) malloc(sizeof(float) * LIST_SIZE);
for (int kkk = 0; kkk < LIST_SIZE; kkk++) {
	k[kkk] = 5.0;
}

cl_int l = 2;
cl_int m = 2;
cl_int n = 2;
cl_int o = 2;
cl_int p = 2;
cl_int q = 2;
int* r = (int*) malloc(sizeof(int) * LIST_SIZE);
for (int rrr = 0; rrr < LIST_SIZE; rrr++) {
	r[rrr] = 5;
}

float* s = (float*) malloc(sizeof(float) * LIST_SIZE);
for (int sss = 0; sss < LIST_SIZE; sss++) {
	s[sss] = 5.0;
}

float* t = (float*) malloc(sizeof(float) * LIST_SIZE);
for (int ttt = 0; ttt < LIST_SIZE; ttt++) {
	t[ttt] = 5.0;
}

FILE* fp;
char* source_str;
size_t source_size;

fp = fopen("rodinia-3.1-particlefilter-likelihood_kernel.cl", "r");
if (!fp) {
fprintf(stderr, "Failed to load kernel.\n");
exit(1);
}
source_str = (char*)malloc(MAX_SOURCE_SIZE);
source_size = fread(source_str, 1, MAX_SOURCE_SIZE, fp);
fclose(fp);

cl_platform_id platform_id = NULL;
cl_device_id device_id = NULL;
cl_uint ret_num_devices;
cl_uint ret_num_platforms;
cl_int ret = clGetPlatformIDs(1, &platform_id, &ret_num_platforms);
ret = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_GPU, 1,
	&device_id, &ret_num_devices);
cl_context context = clCreateContext(NULL, 1, &device_id, NULL, NULL, &ret);

cl_command_queue command_queue = clCreateCommandQueue(context, device_id, 0, &ret);

cl_mem a_mem_obj = clCreateBuffer(context, CL_MEM_READ_WRITE,
	LIST_SIZE * sizeof(float), NULL, &ret);
cl_mem b_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(float), NULL, &ret);
cl_mem c_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(float), NULL, &ret);
cl_mem d_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(float), NULL, &ret);
cl_mem e_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(float), NULL, &ret);
cl_mem f_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(int), NULL, &ret);
cl_mem g_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(int), NULL, &ret);
cl_mem h_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(float), NULL, &ret);
cl_mem i_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(int), NULL, &ret);
cl_mem j_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(float), NULL, &ret);
cl_mem k_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(float), NULL, &ret);
cl_mem r_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(int), NULL, &ret);
cl_mem s_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(float), NULL, &ret);
cl_mem t_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(float), NULL, &ret);
ret = clEnqueueWriteBuffer(command_queue, a_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), a, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, b_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), b, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, c_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), c, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, d_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), d, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, e_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), e, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, f_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(int), f, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, g_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(int), g, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, h_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), h, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, i_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(int), i, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, j_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), j, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, k_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), k, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, r_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(int), r, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, s_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), s, 0, NULL, NULL);
ret = clEnqueueWriteBuffer(command_queue, t_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), t, 0, NULL, NULL);
cl_program program = clCreateProgramWithSource(context, 1,
	(const char**)&source_str, (const size_t*)&source_size, &ret);

ret = clBuildProgram(program, 1, &device_id, NULL, NULL, NULL);

cl_kernel kernel = clCreateKernel(program, "A", &ret);

ret = clSetKernelArg(kernel, 0, sizeof(cl_mem), (void*)&a_mem_obj);
ret = clSetKernelArg(kernel, 1, sizeof(cl_mem), (void*)&b_mem_obj);
ret = clSetKernelArg(kernel, 2, sizeof(cl_mem), (void*)&c_mem_obj);
ret = clSetKernelArg(kernel, 3, sizeof(cl_mem), (void*)&d_mem_obj);
ret = clSetKernelArg(kernel, 4, sizeof(cl_mem), (void*)&e_mem_obj);
ret = clSetKernelArg(kernel, 5, sizeof(cl_mem), (void*)&f_mem_obj);
ret = clSetKernelArg(kernel, 6, sizeof(cl_mem), (void*)&g_mem_obj);
ret = clSetKernelArg(kernel, 7, sizeof(cl_mem), (void*)&h_mem_obj);
ret = clSetKernelArg(kernel, 8, sizeof(cl_mem), (void*)&i_mem_obj);
ret = clSetKernelArg(kernel, 9, sizeof(cl_mem), (void*)&j_mem_obj);
ret = clSetKernelArg(kernel, 10, sizeof(cl_mem), (void*)&k_mem_obj);
ret = clSetKernelArg(kernel, 11, sizeof(cl_int), &l);
ret = clSetKernelArg(kernel, 12, sizeof(cl_int), &m);
ret = clSetKernelArg(kernel, 13, sizeof(cl_int), &n);
ret = clSetKernelArg(kernel, 14, sizeof(cl_int), &o);
ret = clSetKernelArg(kernel, 15, sizeof(cl_int), &p);
ret = clSetKernelArg(kernel, 16, sizeof(cl_int), &q);
ret = clSetKernelArg(kernel, 17, sizeof(cl_mem), (void*)&r_mem_obj);
ret = clSetKernelArg(kernel, 18, sizeof(cl_mem), (void*)&s_mem_obj);
ret = clSetKernelArg(kernel, 19, sizeof(cl_mem), (void*)&t_mem_obj);
size_t global_item_size = LIST_SIZE;
size_t local_item_size = 384;

struct timeval t1, t2;
gettimeofday(&t1, NULL);
ret = clEnqueueNDRangeKernel(command_queue, kernel, 1, NULL,
	&global_item_size, &local_item_size, 0, NULL, NULL);
ret = clFinish(command_queue);
gettimeofday(&t2, NULL);

double elapsedTime;
elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000000.0;
elapsedTime += (t2.tv_usec - t1.tv_usec);

ret = clEnqueueReadBuffer(command_queue, a_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float), a, 0, NULL, NULL);
printf("%.4f", elapsedTime);

ret = clFlush(command_queue);
ret = clFinish(command_queue);
ret = clReleaseKernel(kernel);
ret = clReleaseProgram(program);
ret = clReleaseMemObject(a_mem_obj);
free(a);
ret = clReleaseMemObject(b_mem_obj);
free(b);
ret = clReleaseMemObject(c_mem_obj);
free(c);
ret = clReleaseMemObject(d_mem_obj);
free(d);
ret = clReleaseMemObject(e_mem_obj);
free(e);
ret = clReleaseMemObject(f_mem_obj);
free(f);
ret = clReleaseMemObject(g_mem_obj);
free(g);
ret = clReleaseMemObject(h_mem_obj);
free(h);
ret = clReleaseMemObject(i_mem_obj);
free(i);
ret = clReleaseMemObject(j_mem_obj);
free(j);
ret = clReleaseMemObject(k_mem_obj);
free(k);
ret = clReleaseMemObject(r_mem_obj);
free(r);
ret = clReleaseMemObject(s_mem_obj);
free(s);
ret = clReleaseMemObject(t_mem_obj);
free(t);
ret = clReleaseCommandQueue(command_queue);
ret = clReleaseContext(context);

return 0;

}
