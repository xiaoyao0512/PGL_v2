...
char* g = (char*) malloc(sizeof(char) * LIST_SIZE);
...
fp = fopen("rodinia-3.1-streamcluster-pgain_kernel.cl", "r");
if (!fp) {
...
ret = clEnqueueWriteBuffer(command_queue, g_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(char), g, 0, NULL, NULL);
...
ret = clSetKernelArg(kernel, 10, sizeof(cl_long), &k);
