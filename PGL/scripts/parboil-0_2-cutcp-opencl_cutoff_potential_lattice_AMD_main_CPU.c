...
float4* k = (float4*) malloc(sizeof(float4) * LIST_SIZE);
for (int kkk = 0; kkk < LIST_SIZE; kkk++) {
	k[kkk] = (float4) ( 5.0, 5.0, 5.0, 5.0);
}
...
fp = fopen("parboil-0.2-cutcp-opencl_cutoff_potential_lattice.cl", "r");
if (!fp) {
...
ret = clEnqueueWriteBuffer(command_queue, k_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(float4), k, 0, NULL, NULL);
...
ret = clSetKernelArg(kernel, 4, sizeof(cl_float), &e);
ret = clSetKernelArg(kernel, 5, sizeof(cl_float), &f);
ret = clSetKernelArg(kernel, 6, sizeof(cl_float), &g);
