...
fp = fopen("parboil-0.2-stencil-naive_kernel.cl", "r");
if (!fp) {
...
cl_mem a_mem_obj = clCreateBuffer(context, CL_MEM_READ_WRITE,
	LIST_SIZE * sizeof(floa), NULL, &ret);
...
ret = clSetKernelArg(kernel, 1, sizeof(cl_float), &b);
...
ret = clEnqueueReadBuffer(command_queue, a_mem_obj, CL_TRUE, 0,
	LIST_SIZE * sizeof(floa), a, 0, NULL, NULL);
