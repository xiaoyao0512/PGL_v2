...
fp = fopen("amd-app-sdk-3.0-SimpleConvolution-simpleSeparableConvolutionPass1.cl", "r");
if (!fp) {
...
ret = clSetKernelArg(kernel, 3, sizeof(cl_int2), &d);
ret = clSetKernelArg(kernel, 4, sizeof(cl_int), &e);
ret = clSetKernelArg(kernel, 5, sizeof(cl_int2), &f);
