...
double* g = (double*) malloc(sizeof(double) * LIST_SIZE);
for (int ggg = 0; ggg < LIST_SIZE; ggg++) {
	g[ggg] = 5.0;
}

double* h = (double*) malloc(sizeof(double) * LIST_SIZE);
for (int hhh = 0; hhh < LIST_SIZE; hhh++) {
	h[hhh] = 5.0;
}

double* i = (double*) malloc(sizeof(double) * LIST_SIZE);
...
fp = fopen("npb-3.3-SP-y_solve.cl", "r");
if (!fp) {
...
ret = clSetKernelArg(kernel, 10, sizeof(cl_int), &k);
