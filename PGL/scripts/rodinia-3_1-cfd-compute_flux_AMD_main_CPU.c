...
float3* f = (float3*) malloc(sizeof(float3) * LIST_SIZE);
for (int fff = 0; fff < LIST_SIZE; fff++) {
	f[fff] = (float3) ( 5.0, 5.0, 5.0);
}

float3* g = (float3*) malloc(sizeof(float3) * LIST_SIZE);
for (int ggg = 0; ggg < LIST_SIZE; ggg++) {
	g[ggg] = (float3) ( 5.0, 5.0, 5.0);
}

float3* h = (float3*) malloc(sizeof(float3) * LIST_SIZE);
for (int hhh = 0; hhh < LIST_SIZE; hhh++) {
	h[hhh] = (float3) ( 5.0, 5.0, 5.0);
}

float3* i = (float3*) malloc(sizeof(float3) * LIST_SIZE);
for (int iii = 0; iii < LIST_SIZE; iii++) {
	i[iii] = (float3) ( 5.0, 5.0, 5.0);
}
...
fp = fopen("rodinia-3.1-cfd-compute_flux.cl", "r");
if (!fp) {
...
cl_mem f_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
	LIST_SIZE * sizeof(float3), NULL, &ret);
