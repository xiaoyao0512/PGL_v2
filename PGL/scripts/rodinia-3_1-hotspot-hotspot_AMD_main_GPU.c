...
fp = fopen("rodinia-3.1-hotspot-hotspot.cl", "r");
if (!fp) {
...
ret = clSetKernelArg(kernel, 8, sizeof(cl_float), &i);
ret = clSetKernelArg(kernel, 9, sizeof(cl_float), &j);
ret = clSetKernelArg(kernel, 10, sizeof(cl_float), &k);
ret = clSetKernelArg(kernel, 11, sizeof(cl_float), &l);
ret = clSetKernelArg(kernel, 12, sizeof(cl_float), &m);
